...
public class GuiWaypointManager extends GuiScreen implements RowActions {
    ...
    public static class WaypointList extends GuiSlot {
        ...
        protected void elementClicked(int index, boolean doubleClick, int mouseX, int mouseY) {
            ...
            if (edt.contains(mouseX, mouseY)) { actions.onEdit(wp);
            }
        }
        ...
        @Override protected void overlayBackground(int startY, int endY, int startAlpha, int endAlpha) {
            ...
            worldrenderer.pos(this.left, endY, 0.0F).tex(0.0F, (float)endY / 32.0F).color(64, 64, 64, endAlpha).endVertex();
            worldrenderer.pos(this.left + this.width, endY, 0.0F).tex((float)this.width / 32.0F, (float)endY / 32.0F).color(64, 64, 64, endAlpha).endVertex();
            worldrenderer.pos(this.left + this.width, startY, 0.0F).tex((float)this.width / 32.0F, (float)startY / 32.0F).color(64, 64, 64, startAlpha).endVertex();
            worldrenderer.pos(this.left, startY, 0.0F).tex(0.0F, (float)startY / 32.0F).color(64, 64, 64, startAlpha).endVertex();
            tessellator.draw();
        }
